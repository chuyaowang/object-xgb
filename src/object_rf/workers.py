...
def segment_objects_worker(data: np.ndarray, orig_ndim: int, layer_type: str):
    ...
    if len(props) > 1 and np.any(areas <= 10):
        ...
        print(f'[Object RF] Auto-detected size threshold: {threshold:.2f}')
...
def train_classifier_worker(
    ...
    if not target_labels:
        print('[Object RF] No overlapping annotations found for the objects.')
        yield None
        return
    ...
    probas = classifier.predict_proba(X_all)
    classes = classifier.clf.classes_
...
def apply_rf_worker(
    ...
    """Core logic for applying RF to the full image/stack."""
    classes = classifier.clf.classes_
    n_classes = len(classes)
    ...
    if is_3d:
        ...
        for z in range(total_slices):
            ...
            if existing_df is not None:
                ...
                gen = feature_extractor.generate_features(
                    state_objects, intensity_image=intensity_data, indices=[z]
                )
                feats_df = None
                for val in gen:
                    if isinstance(val, tuple):
        ...
        if feats_df is None:
            gen = feature_extractor.generate_features(
                state_objects, intensity_image=intensity_data
            )
            for val in gen:
                if isinstance(val, tuple):
                    yield (0, 1, val[2])
