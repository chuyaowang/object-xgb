...
class ObjectWidget(QWidget):
            ...
            if (
                ...
                if any(
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        print(
                            f'[Object XGB] Cleared state for: {state["name"]}'
                        )
    ...
    def segment_objects(self):
        ...
        def _on_done(result):
            ...
            print(
                f'[Object XGB] Object segmentation complete. Found {n_objects} objects.'
            )
    ...
    def train_classifier(self):
        ...
        if training_labels.shape != state['objects'].shape:
            print('[Object XGB] Shape mismatch between labels and objects!')
            return
        ...
        def _on_finished():
            ...
            if self._clf_ready:
                ...
                self._on_layer_change()
                print('[Object XGB] Training completed successfully.')
    ...
    def predict_objects(self):
        ...
        pbar = progress(desc='Applying Optimized XGBoost Pipeline')
        ...
        def _on_finished():
            ...
            self.action_buttons.btn_segment.setEnabled(True)
            self._on_layer_change()
            print('[Object XGB] Optimized XGBoost prediction complete.')
    ...
    def save_labels(self):
        ...
        if manual_labels_layer is not None:
            ...
            print(f'[Object XGB] Saved labels to {save_path}')
    ...
    def _save_state_outputs(self, state, prob_key, suffix):
        ...
        classes = self.clf.model.classes_
        ...
        print(
            f'[Object XGB] Saved {suffix} class map and probabilities to {save_dir}'
        )

    def save_features(self):
        """Saves a complete XGBoost report (metadata + true labels + predictions)."""
        state = self.state_manager.get_state(self.state_manager.current_image)
        if state and state['full_feature_table'] is not None:
            ...
            df = state['full_feature_table'].copy()

            # 1. Generate the report using the labels already stored in the table
            # (true_label was populated during the Train step)
            report_df = self.clf.get_report(df, df['true_label'], df)

            # 2. Merge metadata/preds with original features
            final_export = pd.concat(
                [
                    report_df,
                    df.drop(columns=['label', 'slice_id', 'true_label']),
                ],
                axis=1,
            )

            save_path = save_dir / f'{name}_full_analysis_report.csv'
            final_export.to_csv(save_path, index=False)
            print(f'[Object XGB] Saved full analysis report to {save_path}')
    ...
    def save_model(self):
        ...
        default_path = str(save_dir / 'object_xgboost_pipeline.joblib')

        save_path, _ = QFileDialog.getSaveFileName(
            self, 'Save Model Pipeline', default_path, 'Model (*.joblib)'
        )
        if save_path:
            dump(self.clf, save_path)
            print(f'[Object XGB] Model pipeline saved to {save_path}')

    def load_model(self):
        load_path, _ = QFileDialog.getOpenFileName(
            self, 'Load Model Pipeline', '', 'Model (*.joblib)'
        )
        if load_path:
            self.clf = load(load_path)
            self._clf_ready = True
            self._on_layer_change()
            print(f'[Object XGB] Model pipeline loaded from {load_path}')
